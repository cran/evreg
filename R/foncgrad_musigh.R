...
foncgrad_musigh<-function(theta,y,eps,lambda,nu){
  ...
  PL<-A+B
  EHS<-eps*hx*sig2x
  Phi3<-pnorm(y+EHS,mux,sig1)
  Phi4<-pnorm(y-EHS,mux,sig1)
  BEL<-pmax(0,PL-pl1*Phi3-pl2*(1-Phi4))
  loss <- mean(-lambda*log(BEL+nu)-(1-lambda)*log(PL))
  ...
  phi<-dnorm((y-mux+EHS)/sig1)
  dPhidh <- phi * (eps*sigx -0.5*sigx*Z2^(-1)*(y-mux+EHS))/Z
  dPhidmu <- -phi/sig1
  dPhidsig2 <- phi*(eps*hx-0.5*(1/sig2x)*
                      (1/Z2)*(2*hx*sig2x+1)*
                      (y-mux+EHS))/sig1
  ...
  dPhidsig2 <- phi*(-eps*hx-0.5*(1/sig2x)*
                      (1/Z2)*(2*hx*sig2x+1)*
                      (y-mux-EHS))/sig1
  gradPhi4<-cbind(dPhidmu,dPhidsig2,dPhidh)

  A<- gradpl1*Phi3+pl1*gradPhi3
  B<- gradpl2*(1-Phi4)-pl2*gradPhi4
